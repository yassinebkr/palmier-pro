# PalmierPro / PalmierWin

This repo is the PalmierWin fork: the upstream **Palmier Pro** macOS app (Swift 6.2, SwiftUI + AppKit, AVFoundation; macOS 26, arm64, non-sandboxed Developer ID app) plus **PalmierWin**, a Windows-native AI video editor built on the same core (C#/Avalonia shell + Swift media engine, Windows x64, Vulkan + FFmpeg). The shared portable editor model lives in `Sources/PalmierCore/` and is consumed by both apps.
...
Windows app (`PalmierWin/`):
```powershell
.\fetch-deps.ps1          # native deps once
.\build.bat               # Swift engine + PalmierCoreHost.dll (+ spike harness)
dotnet build Shell        # C# shell
dotnet test Shell/PalmierShell.sln   # shell + interop suite (this is the Windows CI gate)
...
- `Sources/PalmierCore/` — portable model + engines (Timeline/Clip/Track, Ripple/Overwrite engines, curves). Must stay Apple-free: no AppKit/AVFoundation/CoreImage imports. Windows CI type-checks and tests it in isolation. **Two-tree rule:** the Windows build consumes it as `PalmierWin/Sources/PalmierCore` — a gitignored local copy (CI symlinks it instead; symlinks need admin on dev machines). Edit ONLY the tracked `Sources/PalmierCore`, then sync with `cp Sources/PalmierCore/Timeline.swift PalmierWin/Sources/PalmierCore/Timeline.swift` (per file, or the whole dir). Never edit the copy directly — the two trees disagreeing has already broken a build once.
- `PalmierWin/Sources/PalmierWin/` — the Swift media engine: FFmpeg decode/encode, Vulkan compositor, SPIR-V effect kernels (`PalmierWin/Shaders/`, regenerated by `build-shaders.ps1`), audio (vendored miniaudio/WASAPI), text (stb_truetype).
- `PalmierWin/Sources/PalmierCoreHost/` — the `@_cdecl` C ABI the shell calls in-proc. ABI rules: opaque handles registered in `HandleRegistry` (double/foreign destroy = no-op), value types and JSON across the boundary, events polled not callbacks, teardown in dependency order (swapchain → device → instance), a Swift owner must outlive every in-flight command buffer that references its object.
- `PalmierWin/CCrashGuard/` — native vectored crash handler linked into PalmierCoreHost: on a fatal native fault it spawns the shell as `--crash-report` (a healthy process) to write the crash log + minidump. Never register a managed vectored handler — entering managed code on an arbitrary faulting thread is a fatal CLR reverse-pinvoke transition under GC (0x80131506).
- `PalmierWin/Shell/` — the C#/.NET 9 Avalonia app (`PalmierShell`) + xunit suites (`PalmierShell.Tests`) + the interop spike. No native UI frameworks (no WPF/WinUI) and no COM anywhere in the port.
- `PalmierWin/Sources/palmierwin-spike/` — engine dev harness with GPU readback checks (effect kernels, export round trip). The ImGui editor UI in it is legacy harness only; the product UI is the Avalonia shell.

## Windows shell conventions

- UI styling uses the AppTheme resource tokens in `PalmierShell/Theme/AppTheme.axaml` (ported from the macOS `AppTheme.swift`) — never hardcode colors/spacing; add a token when one is missing.
