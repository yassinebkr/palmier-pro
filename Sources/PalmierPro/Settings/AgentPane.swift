            ...
            SettingsSection(title: "AI Chat") {
                VStack(alignment: .leading, spacing: AppTheme.Spacing.xxl) {
                    ...
                    APIKeySection(
                        ...
                        delete: { await Task.detached(priority: .userInitiated) { OpenAIKeychain.delete() }.value }
                    )
                }
            }
...
/// One provider's API-key field. The keychain load/save/delete closures are
/// the only per-provider customization; the field, masking, and trailing
/// Save/Delete controls are shared so every provider looks the same.
private struct APIKeySection: View {
    ...
    let load: @Sendable () async -> String
    let save: @Sendable (String) async -> Void
    ...
    private var fieldBox: some View {
        SecureField(fieldPlaceholder, text: $draft)
