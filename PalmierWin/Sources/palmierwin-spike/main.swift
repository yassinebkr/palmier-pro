            ...
            if let dev = VulkanDevice.create(instance: instance) {
                ...
                if let win = Win32Window(title: "Palmier Pro Windows", width: 1280, height: 720) {
                    ...
                    if let swap = VulkanSwapchain(device: dev, instance: instance, window: win) {
                        ...
                        if VulkanPipeline(device: dev, renderPass: swap.renderPass, extent: swap.extent) != nil {
                            print("Vulkan pipeline: created OK (shaders loaded, descriptor layout + pipeline built)")
                        } else {
                            print("Vulkan pipeline: FAILED to create")
                        }

                        // 9) FFmpeg decode + Vulkan texture upload: decode one
                        //    frame from a test clip, convert to BGRA, upload to
                        //    a GPU texture the pipeline could sample. Proves the
                        //    end-to-end media path on Windows without presenting.
                        //    The test clip is generated by make-test-media.ps1
                        //    (not committed) — the step is skipped if absent so
                        //    CI (no GPU, no clip) doesn't fail.
                        ...
                        if FileManager.default.fileExists(atPath: clipPath) {
                            do {
                                ...
                                if let bgra = try decoder.nextBGRAFrame() {
                                    print("FFmpeg decoder: decoded frame, \(bgra.count) bytes (expected \(decoder.info.width * decoder.info.height * 4))")
                                    if let tex = VulkanTexture(device: dev, width: UInt32(decoder.info.width), height: UInt32(decoder.info.height)) {
                            ...
                            print("FFmpeg decode+upload: skipped (no \(clipPath) — run PalmierWin/make-test-media.ps1)")
                        }
                    } else {
